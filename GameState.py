...
class GameState:
    ...
    def updateMortgagedProperty(self, p):
        if self.turn_id % 2 == 0:
            self.property_status[p.id].status = Status.OWNED_P1_MORTGAGED
        else:
            self.property_status[p.id].status = Status.OWNED_P2_MORTGAGED
        self.addCash(self.property_status[p.id].price)

    def updateBuiltHouse(self, p):
        num = self.property_status[p.id].status.value
        if abs(num) >= 6:
            # Property is mortgaged or there is a hotel, cannot build
            return False
        if self.turn_id %2 == 0:
            self.property_status[p.id].status = Status(num + 1)
        else:
            self.property_status[p.id].status = Status(num - 1)
    ...
    def updateBuiltHotel(self, p):
        ...
        if abs(num) != 5:
            ...
            return False
        if self.turn_id %2 == 0:
            self.property_status[p.id].status = Status.OWNED_P1_HOTEL
        else:
            self.property_status[p.id].status = Status.OWNED_P2_HOTEL
        self.deductCash(p.build_cost)
        return True
